...
def test_contact_us_form(base_url, browser):
    ...
    with allure.step("5. Enter name, email, subject, and message"):
        contact.fill_form(name, email, subject, message)
        take_screenshot(page, "Filled Contact Form")

    with allure.step("6. Upload file"):
        contact.upload_file(file_path)
        take_screenshot(page, "File Uploaded")

    with allure.step("7. Click 'Submit' and accept the confirmation dialog"):
    ...
    with allure.step("8. Verify success message 'Success! Your details have been submitted successfully.' is visible"):
    ...
    with allure.step("9. Click 'Home' button and verify navigation to home page"):
